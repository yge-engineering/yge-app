// Job-anchored AR snapshot.
//
// Plain English: for one job, as-of today, sum every AR
// invoice's billed/paid/open/retention; bucket open by age;
// count invoices. Drives the right-now per-job customer
...
export interface JobArSnapshotResult {
  asOf: string;
  jobId: string;
  totalCents: number;
  paidCents: number;
...
export interface JobArSnapshotInputs {
  jobId: string;
...
export function buildJobArSnapshot(
  inputs: JobArSnapshotInputs,
): JobArSnapshotResult {
