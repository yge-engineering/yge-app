// Job-anchored AP snapshot.
//
// Plain English: for one job, as-of today, sum every AP
// invoice tagged to the job: billed/paid (non-voided)/open;
// bucket open by age; count distinct vendors. Drives the
// right-now per-job vendor cost + payable overview.
//
...
export interface JobApSnapshotResult {
  asOf: string;
  jobId: string;
  totalCents: number;
  paidCents: number;
...
export interface JobApSnapshotInputs {
  jobId: string;
...
export function buildJobApSnapshot(inputs: JobApSnapshotInputs): JobApSnapshotResult {
