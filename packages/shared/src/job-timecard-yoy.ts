// Job-anchored timecard year-over-year.
//
// Plain English: for one job, collapse two years of timecard
// activity into a comparison: hours-on-job + proportional
// daily/weekly OT, distinct employees, plus deltas.
//
...
export interface JobTimecardYoyResult {
  jobId: string;
  priorYear: number;
  currentYear: number;
  priorHoursOnJob: number;
  priorDailyOt: number;
  priorWeeklyOt: number;
  priorDistinctEmployees: number;
  currentHoursOnJob: number;
...
export interface JobTimecardYoyInputs {
  jobId: string;
  timeCards: TimeCard[];
  currentYear: number;
}
...
export function buildJobTimecardYoy(inputs: JobTimecardYoyInputs): JobTimecardYoyResult {
