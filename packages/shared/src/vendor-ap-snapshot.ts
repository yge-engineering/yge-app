// Vendor-anchored AP snapshot.
//
// Plain English: for one vendor (matched by canonicalized
// name), as-of today, sum every AP invoice from them: billed/
// paid (non-voided)/open, age buckets, distinct jobs they've
// billed against. Drives the right-now per-vendor payable
// overview.
//
...
export interface VendorApSnapshotResult {
  asOf: string;
  vendorName: string;
  totalCents: number;
  paidCents: number;
...
export interface VendorApSnapshotInputs {
  vendorName: string;
...
export function buildVendorApSnapshot(
  inputs: VendorApSnapshotInputs,
): VendorApSnapshotResult {
