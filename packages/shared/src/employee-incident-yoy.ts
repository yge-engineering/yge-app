// Employee-anchored incident year-over-year.
//
// Plain English: for one employee, collapse two years of
// incidents naming them into a comparison: counts, days lost,
// classification + outcome mix, distinct jobs, plus deltas.
//
...
export interface EmployeeIncidentYoyResult {
  employeeId: string;
  priorYear: number;
  currentYear: number;
  priorTotal: number;
...
export interface EmployeeIncidentYoyInputs {
  employeeId: string;
  incidents: Incident[];
  currentYear: number;
}

export function buildEmployeeIncidentYoy(
  inputs: EmployeeIncidentYoyInputs,
): EmployeeIncidentYoyResult {
