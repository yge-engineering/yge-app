...
function statusTone(status: ChangeOrderStatus): { border: string; bg: string; text: string } {
  switch (status) {
    case 'APPROVED':
    case 'EXECUTED':
...
export default function ChangeOrdersScreen() {
  ...
  const [cos, setCos] = useState<ChangeOrderRow[]>([]);

  async function load() {
    setError(null);
    try {
      const json = await getJson<{ changeOrders: ChangeOrderRow[] }>('/api/change-orders');
      setCos(json.changeOrders ?? []);
    } catch (err) {
  ...
  return (
    ...
    >
      <Text style={styles.h1}>Change Orders</Text>
      <Text style={styles.sub}>{cos.length} total</Text>
          ...
          <View key={c.id} style={styles.card}>
            <View style={styles.titleRow}>
              <Text style={styles.cardTitle} numberOfLines={1}>
                {c.changeOrderNumber} · {c.subject}
              </Text>
              <View style={[styles.pill, { borderColor: tone.border, backgroundColor: tone.bg }]}>
                <Text style={[styles.pillText, { color: tone.text }]}>{c.status.replace('_', ' ')}</Text>
              </View>
            </View>
            {c.description ? (
