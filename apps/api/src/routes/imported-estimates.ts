...
importedEstimatesRouter.get('/:id/sections-summary.csv', async (req, res, next) => {
  try {
    const ie = await getImportedEstimate(req.params.id);
    if (!ie) return res.status(404).json({ error: 'Imported estimate not found' });
    ...
    const lines: string[] = [];
    lines.push('section,lines,direct,bid');
    for (const s of sections) {
      lines.push([
        esc(s.name),
        esc(s.lines),
        esc((s.directCents / 100).toFixed(2)),
        esc((s.bidCents / 100).toFixed(2)),
      ].join(','));
    }
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename="${ie.jobNumber}-sections.csv"`);
    res.send(lines.join('\n'));
  } catch (err) { next(err); }
});
