...
export function FuelReductionScopeBanner({ draft }: Props) {
  const result = checkFuelReductionScope(draft);
  if (!result.isFuelReductionJob) return null;
    ...
    <section className="rounded-lg border border-amber-300 bg-amber-50 p-4 text-amber-900">
      <header className="mb-3">
        <h2 className="text-sm font-bold uppercase tracking-wide">
          ⚠ Fuel-reduction scope check — {result.missingItems.length} item
          {result.missingItems.length === 1 ? '' : 's'} missing
        </h2>
        <p className="mt-1 text-xs text-amber-800">
          This draft pattern-matches as CAL FIRE / vegetation management /
          fuel-reduction work ({result.detectedKeywords.slice(0, 3).join(', ')}
          {result.detectedKeywords.length > 3 ? ', ...' : ''}). The items
          below typically ride along with the headline acreage — easy to
          miss when the spec only itemizes the mastication price per acre.
        </p>
      </header>
