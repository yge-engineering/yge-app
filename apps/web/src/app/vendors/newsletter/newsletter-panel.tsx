...
export function VendorNewsletterPanel() {
  const [bcc, setBcc] = useState<string>('');
  const [total, setTotal] = useState<number>(0);
  const [subject, setSubject] = useState('YGE — quick note to our vendor list');
  const [body, setBody] = useState(
    "Hi,\n\nQuick note to keep in touch with our vendor and supplier network. If anything has changed on your end (pricing, contact info, lead times, certifications), please let us know.\n\nThanks,\nRyan Young\nYoung General Engineering, Inc.\n707-599-9921",
  );

  useEffect(() => {
    fetch(`${apiBaseUrl()}/api/vendors/email-list`, { cache: 'no-store' })
  ...
  return (
    <div className="space-y-4">
      <div className="rounded border border-gray-200 bg-white p-3 shadow-sm">
        <div className="text-xs text-gray-500">Will BCC {total} vendor{total === 1 ? '' : 's'}.</div>
      </div>
