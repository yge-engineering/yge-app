...
interface Vendor {
  ...
  kind?: string;
  state?: string | null;
  data?: { legalName?: string; kind?: string; state?: string | null };
}
...
export function MissingStateTable() {
  ...
  const rows = vendors.filter((v) => !st(v).trim());

  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
        Every vendor has a state. Nice.
      </p>
    );
  }
