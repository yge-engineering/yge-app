...
interface Vendor {
  ...
  city?: string | null;
  state?: string | null;
  data?: { legalName?: string; zip?: string | null; city?: string | null; state?: string | null };
}
...
export function ByZipDetail() {
  ...
  return (
    <div className="space-y-3">
      {sorted.map(([zip, list]) => (
        <details key={zip} className="rounded border border-gray-200 bg-white shadow-sm">
          <summary className="flex cursor-pointer items-center justify-between px-3 py-2 text-sm">
            <span className="font-mono font-semibold">{zip}</span>
          ...
          <ul className="divide-y divide-gray-100 px-3 pb-2 text-sm">
            {list.map((v) => (
              <li key={v.id} className="flex items-center justify-between gap-3 py-1.5">
                ...
                <span className="font-mono text-[10px] text-gray-500">{ci(v)} {st(v)}</span>
              </li>
            ))}
          </ul>
        </details>
