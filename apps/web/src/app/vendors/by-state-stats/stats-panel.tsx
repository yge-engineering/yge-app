...
interface Vendor { id: string; state?: string | null; data?: { state?: string | null } }

function st(v: Vendor): string { return ((v.state ?? v.data?.state) ?? '').trim().toUpperCase() || '(unknown)'; }

export function ByStateStats() {
          ...
          {rows.map(([state, count]) => (
            <tr key={state} className="border-t border-gray-100">
              ...
              <td className="px-3 py-2 text-right">
                <Link href="/vendors/by-state-detail" className="text-xs text-yge-blue-700 hover:underline">view</Link>
              </td>
            </tr>
          ))}
        </tbody>
