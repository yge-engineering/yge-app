...
interface Vendor {
  ...
  email?: string;
  phone?: string;
  data?: { legalName?: string; kind?: string; email?: string; phone?: string };
}
...
export function NoContactTable() {
  ...
  const rows = vendors.filter((v) => !em(v).includes('@') && !ph(v).trim());

  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
        Every vendor has at least one contact method. Nice.
      </p>
    );
  }
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-gray-200 bg-white shadow-sm">
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map((v) => (
            <tr key={v.id} className="border-t border-gray-100 bg-red-50">
