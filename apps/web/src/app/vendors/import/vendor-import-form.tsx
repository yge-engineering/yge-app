...
export function VendorImportForm() {
  ...
  async function submit(dryRun: boolean) {
    ...
    try {
      ...
      fd.append('file', file);
      const url = `${apiBaseUrl()}/api/vendors/import-csv${dryRun ? '?dryRun=1' : ''}`;
  ...
  return (
    <div>
      <div className="mb-4 rounded-md border border-gray-200 bg-blue-50 p-3 text-xs text-blue-900">
        <strong>Required columns:</strong> <code>legalName</code>, <code>kind</code> (SUBCONTRACTOR | SUPPLIER | RENTAL | LABOR | SERVICE | OTHER).{' '}
        <strong>Optional:</strong> dbaName, contactName, phone, email, tradeSpecialty, licenseNumber, paymentTerms, is1099Reportable, notes.{' '}
        <a href={`${apiBaseUrl()}/api/vendors/export.csv`} download className="text-yge-blue-700 hover:underline">
