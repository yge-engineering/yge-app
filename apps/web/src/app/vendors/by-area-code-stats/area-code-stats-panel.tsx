...
export function AreaCodeStatsPanel() {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
      <Stat label="Top area code" value={top ? top[0] : '—'} sub={top ? `${top[1]} vendors` : 'no phones yet'} />
      <Stat label="Unique area codes" value={String(counts.size)} sub={`across ${vendors.length - missing} phones`} />
      <Stat label="Missing phone" value={String(missing)} sub={`of ${vendors.length} vendors`} tone={missing > 0 ? 'warn' : 'good'} />
    </div>
  );
}
