...
interface Rate { id: string; kind?: 'OWNED' | 'RENTAL' | string; hourlyCents?: number | null }
...
function buildStats(rates: Rate[]): Stat[] {
  ...
  return [...map.entries()].map(([kind, s]) => ({
    kind,
    count: s.count,
    minCents: Number.isFinite(s.min) ? s.min : 0,
    maxCents: s.max,
    avgCents: s.count > 0 ? Math.round(s.sum / s.count) : 0,
  })).sort((a, b) => b.count - a.count);
}

export function OwnedVsRentalTable() {
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-gray-200 bg-white shadow-sm">
      <table className="w-full text-sm">
        ...
        <tbody>
          {stats.map((s) => (
            <tr key={s.kind} className="border-t border-gray-100">
              <td className="px-3 py-2 font-semibold">{s.kind}</td>
              <td className="px-3 py-2 text-right font-mono">{s.count}</td>
              <td className="px-3 py-2 text-right font-mono"><Money cents={s.minCents} /></td>
              <td className="px-3 py-2 text-right font-mono"><Money cents={s.avgCents} /></td>
              <td className="px-3 py-2 text-right font-mono"><Money cents={s.maxCents} /></td>
            </tr>
          ))}
        </tbody>
      </table>
