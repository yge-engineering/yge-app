...
export function EquipmentImportForm() {
  ...
  async function submit(dryRun: boolean) {
    ...
    try {
      ...
      fd.append('file', file);
      const url = `${apiBaseUrl()}/api/equipment-rates/import-csv${dryRun ? '?dryRun=1' : ''}`;
  ...
  return (
    <div>
      <div className="mb-4 rounded-md border border-gray-200 bg-blue-50 p-3 text-xs text-blue-900">
        <a href={`${apiBaseUrl()}/api/equipment-rates/export.csv`} download className="text-yge-blue-700 hover:underline">
          Download current equipment as a starter CSV →
        </a>
      </div>
