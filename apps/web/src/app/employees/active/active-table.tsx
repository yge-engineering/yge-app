...
export function ActiveEmployeesTable() {
        ...
        <tbody>
          {rows.map((e) => (
            <tr key={e.id} className="border-t border-gray-100">
              ...
              <td className="px-3 py-2 font-mono text-xs text-gray-700">{e.phone ?? '—'}</td>
              <td className="px-3 py-2 text-xs text-gray-700">{e.email ?? '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
