// /pcos — Potential Change Order log.

import Link from 'next/link';
import {
  computePcoRollup,
  daysAwaitingResponse,
  formatUSD,
  isOpenExposure,
  pcoOriginLabel,
  pcoStatusLabel,
  type Pco,
  type PcoStatus,
} from '@yge/shared';

const STATUSES: PcoStatus[] = [
  'DRAFT',
  'SUBMITTED',
  'UNDER_REVIEW',
  'APPROVED_PENDING_CO',
  'REJECTED',
  'WITHDRAWN',
  'CONVERTED_TO_CO',
];

function apiBaseUrl(): string {
  return (
    process.env.API_URL ?? process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:4000'
  );
}

async function fetchPcos(filter: { status?: string; jobId?: string }): Promise<Pco[]> {
  const url = new URL(`${apiBaseUrl()}/api/pcos`);
  if (filter.status) url.searchParams.set('status', filter.status);
  if (filter.jobId) url.searchParams.set('jobId', filter.jobId);
  const res = await fetch(url.toString(), { cache: 'no-store' });
  if (!res.ok) return [];
  return ((await res.json()) as { pcos: Pco[] }).pcos;
}
async function fetchAll(): Promise<Pco[]> {
  const res = await fetch(`${apiBaseUrl()}/api/pcos`, { cache: 'no-store' });
  if (!res.ok) return [];
  return ((await res.json()) as { pcos: Pco[] }).pcos;
}

export default async function PcoListPage({
  searchParams,
}: {
  searchParams: { status?: string; jobId?: string };
}) {
  const [pcos, all] = await Promise.all([fetchPcos(searchParams), fetchAll()]);
  const rollup = computePcoRollup(all);

  function buildHref(overrides: Partial<{ status?: string }>): string {
    const params = new URLSearchParams();
    const merged = { ...searchParams, ...overrides };
    if (merged.status) params.set('status', merged.status);
    if (merged.jobId) params.set('jobId', merged.jobId);
    const q = params.toString();
    return q ? `/pcos?${q}` : '/pcos';
  }

  return (
    <main className="mx-auto max-w-6xl p-8">
      <div className="mb-6 flex items-center justify-between">
        <Link href="/" className="text-sm text-yge-blue-500 hover:underline">
          &larr; Home
        </Link>
        <Link
          href="/pcos/new"
          className="rounded bg-yge-blue-500 px-3 py-1 text-sm font-medium text-white hover:bg-yge-blue-700"
        >
          + New PCO
        </Link>
      </div>

      <h1 className="text-3xl font-bold text-yge-blue-500">Potential Change Orders</h1>
      <p className="mt-2 text-gray-700">
        Money + schedule already worked or about to be worked but not yet
        contracted for. PCOs become formal change orders once approved.
      </p>

      <section className="mt-6 grid gap-4 sm:grid-cols-4">
        <Stat label="Total" value={rollup.total} />
        <Stat
          label="Open exposure ($)"
          value={formatUSD(rollup.openExposureCents)}
          variant={rollup.openExposureCents > 0 ? 'warn' : 'ok'}
        />
        <Stat
          label="Approved-pending-CO ($)"
          value={formatUSD(rollup.approvedPendingCoCents)}
          variant={rollup.approvedPendingCoCents > 0 ? 'warn' : 'ok'}
        />
        <Stat
          label="Oldest awaiting reply"
          value={`${rollup.oldestAwaitingDays} days`}
          variant={rollup.oldestAwaitingDays > 30 ? 'bad' : rollup.oldestAwaitingDays > 14 ? 'warn' : 'ok'}
        />
      </section>

      <section className="mt-6 flex flex-wrap items-center gap-2 rounded-lg border border-gray-200 bg-white p-3 shadow-sm">
        <span className="text-xs uppercase tracking-wide text-gray-500">Status:</span>
        <Link
          href={buildHref({ status: undefined })}
          className={`rounded px-2 py-1 text-xs ${!searchParams.status ? 'bg-yge-blue-500 text-white' : 'border border-gray-300 text-gray-700 hover:bg-gray-50'}`}
        >
          All
        </Link>
        {STATUSES.map((s) => (
          <Link
            key={s}
            href={buildHref({ status: s })}
            className={`rounded px-2 py-1 text-xs ${searchParams.status === s ? 'bg-yge-blue-500 text-white' : 'border border-gray-300 text-gray-700 hover:bg-gray-50'}`}
          >
            {pcoStatusLabel(s)}
          </Link>
        ))}
      </section>

      {pcos.length === 0 ? (
        <div className="mt-6 rounded border border-gray-200 bg-gray-50 p-6 text-sm text-gray-600">
          No PCOs yet. Click <em>New PCO</em>.
        </div>
      ) : (
        <div className="mt-6 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
              <tr>
                <th className="px-4 py-2">PCO #</th>
                <th className="px-4 py-2">Title</th>
                <th className="px-4 py-2">Origin</th>
                <th className="px-4 py-2">Status</th>
                <th className="px-4 py-2 text-right">Cost ($)</th>
                <th className="px-4 py-2 text-right">Days</th>
                <th className="px-4 py-2 text-right">Awaiting</th>
                <th className="px-4 py-2"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {pcos.map((p) => {
                const wait = daysAwaitingResponse(p);
                const flag = wait > 30;
                return (
                  <tr key={p.id} className={flag ? 'bg-red-50' : ''}>
                    <td className="px-4 py-3 font-mono text-xs text-gray-700">
                      {p.pcoNumber}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-900">
                      <div className="font-medium">{p.title}</div>
                      <div className="text-xs text-gray-500 line-clamp-1">{p.description}</div>
                    </td>
                    <td className="px-4 py-3 text-xs text-gray-700">
                      {pcoOriginLabel(p.origin)}
                    </td>
                    <td className="px-4 py-3 text-xs">
                      <span
                        className={`inline-block rounded px-1.5 py-0.5 font-semibold ${
                          p.status === 'APPROVED_PENDING_CO'
                            ? 'bg-yellow-100 text-yellow-800'
                            : p.status === 'CONVERTED_TO_CO'
                              ? 'bg-green-100 text-green-800'
                              : p.status === 'REJECTED'
                                ? 'bg-red-100 text-red-800'
                                : isOpenExposure(p)
                                  ? 'bg-blue-100 text-blue-800'
                                  : 'bg-gray-100 text-gray-700'
                        }`}
                      >
                        {pcoStatusLabel(p.status)}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-sm">
                      {formatUSD(p.costImpactCents)}
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-xs text-gray-700">
                      {p.scheduleImpactDays > 0 ? `+${p.scheduleImpactDays}d` : p.scheduleImpactDays}
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-xs">
                      {wait > 0 ? (
                        <span className={flag ? 'font-semibold text-red-700' : 'text-gray-700'}>
                          {wait}d
                        </span>
                      ) : (
                        <span className="text-gray-400">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-right text-sm">
                      <Link href={`/pcos/${p.id}`} className="text-yge-blue-500 hover:underline">
                        Open
                      </Link>
                    </td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </div>
      )}
    </main>
  );
}

function Stat({
  label,
  value,
  variant = 'neutral',
}: {
  label: string;
  value: string | number;
  variant?: 'neutral' | 'ok' | 'warn' | 'bad';
}) {
  const cls =
    variant === 'ok'
      ? 'border-green-200 bg-green-50 text-green-800'
      : variant === 'warn'
        ? 'border-yellow-200 bg-yellow-50 text-yellow-800'
        : variant === 'bad'
          ? 'border-red-200 bg-red-50 text-red-800'
          : 'border-gray-200 bg-white text-gray-900';
  return (
    <div className={`rounded-lg border p-4 shadow-sm ${cls}`}>
      <div className="text-xs uppercase tracking-wide opacity-70">{label}</div>
      <div className="mt-1 text-xl font-bold">{value}</div>
    </div>
  );
}
