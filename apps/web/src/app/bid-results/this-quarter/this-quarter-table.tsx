...
import { winningAmountCents, ygeBid, type BidResult } from '@yge/shared';
...
export function ThisQuarterTable() {
  ...
  const startIso = qStart.toISOString().slice(0, 10);
  const endIso = qEnd.toISOString().slice(0, 10);

  const rows = results
    .filter((r) => (r.bidOpenedAt ?? '') >= startIso && (r.bidOpenedAt ?? '') < endIso)
    .sort((a, b) => b.bidOpenedAt.localeCompare(a.bidOpenedAt));

  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
        No bid results in {year} Q{q} yet.
      </p>
    );
  }
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map((r) => {
            ...
            return (
              <tr key={r.id} className={`border-t border-gray-100 ${r.outcome === 'WON_BY_YGE' ? 'bg-emerald-50' : ''}`}>
                ...
                <td className="px-3 py-2 font-mono text-xs text-gray-700">{r.outcome}</td>
