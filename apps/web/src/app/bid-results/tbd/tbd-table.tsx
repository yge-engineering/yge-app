...
export function TbdTable() {
  ...
  const rows = results
    .filter((r) => r.outcome === 'TBD')
    .sort((a, b) => b.bidOpenedAt.localeCompare(a.bidOpenedAt));

  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
        No bid results currently in TBD status — clean slate.
      </p>
    );
  }
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map((r) => {
            const yge = ygeBid(r);
            return (
              <tr key={r.id} className="border-t border-gray-100 bg-amber-50">
