...
interface Job { id: string; projectName: string; ownerAgency?: string | null }
...
export function ByAgencyDetail() {
  ...
  for (const r of results) {
    ...
    const agency = (job?.ownerAgency ?? '').trim() || '(unknown)';
    if (!groups.has(agency)) groups.set(agency, []);
    groups.get(agency)!.push(r);
  }
          ...
          <details key={agency} className="rounded border border-gray-200 bg-white shadow-sm">
            <summary className="flex cursor-pointer items-center justify-between px-3 py-2 text-sm">
              ...
              <span className="text-xs text-gray-600">
                {list.length} bid{list.length === 1 ? '' : 's'} · {wins} win{wins === 1 ? '' : 's'}
              </span>
            </summary>
            <ul className="divide-y divide-gray-100 px-3 pb-2 text-sm">
              {[...list].sort((a, b) => b.bidOpenedAt.localeCompare(a.bidOpenedAt)).map((r) => (
                <li key={r.id} className="flex items-center justify-between gap-3 py-1.5">
                  ...
                  <span className="font-mono text-[10px] text-gray-500">{r.bidOpenedAt} · {r.outcome}</span>
                </li>
              ))}
            </ul>
          </details>
