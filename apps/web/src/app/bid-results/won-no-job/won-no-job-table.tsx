...
export function WonNoJobTable() {
  ...
  const rows = results
    .filter((r) => r.outcome === 'WON_BY_YGE' && !jobIds.has(r.jobId))
    .sort((a, b) => b.bidOpenedAt.localeCompare(a.bidOpenedAt));

  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
        Every recorded win has a matching job record. Clean.
      </p>
    );
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-xs text-gray-500">
            ...
            <th className="px-3 py-2">Bid result</th>
            <th className="px-3 py-2">Job ID on record</th>
            ...
            return (
              <tr key={r.id} className="border-t border-gray-100">
                ...
                <td className="px-3 py-2">
                  <Link href={`/bid-results/${r.id}`} className="font-semibold text-yge-blue-700 hover:underline">{r.id}</Link>
                </td>
                <td className="px-3 py-2 font-mono text-xs text-amber-700">{r.jobId}</td>
