...
interface Summary {
  groups: number; bidders: number; created: number; updated: number; skipped: number;
  errors: Array<{ row: number; reason: string }>; dryRun: boolean;
}

export function BidResultImportForm() {
  ...
  async function submit(dryRun: boolean) {
    ...
    try {
      ...
      fd.append('file', file);
      const url = `${apiBaseUrl()}/api/bid-results/import-csv${dryRun ? '?dryRun=1' : ''}`;
  ...
  return (
    <div>
      <div className="mb-4 rounded-md border border-gray-200 bg-blue-50 p-3 text-xs text-blue-900">
        <strong>Required:</strong> jobNumber, bidOpenedAt (yyyy-mm-dd), bidderName, bidderAmount.{' '}
        <strong>Optional:</strong> outcome (WON_BY_YGE / WON_BY_OTHER / NO_AWARD / TBD), bidderIsYge (true/false), bidderNotes.{' '}
        <a href={`${apiBaseUrl()}/api/bid-results/export.csv`} download className="text-yge-blue-700 hover:underline">
          Download starter CSV →
        </a>
      </div>
      <input type="file" accept=".csv,text/csv" onChange={(e) => setFile(e.target.files?.[0] ?? null)} className="mb-3 block text-sm" />
      ...
      {result && (
        <div className="mt-3 rounded-md border border-gray-200 bg-gray-50 p-3 text-xs">
          <div className="font-semibold">{result.dryRun ? 'Dry-run' : 'Imported'}</div>
          <div>{result.bidders} bidders → {result.groups} results · {result.created} created · {result.updated} updated · {result.skipped} skipped</div>
          {result.errors.length > 0 && (
            <ul className="mt-2 list-inside list-disc text-red-700">
              {result.errors.slice(0, 10).map((e, i) => <li key={i}>{e.reason}</li>)}
            </ul>
          )}
        </div>
      )}
