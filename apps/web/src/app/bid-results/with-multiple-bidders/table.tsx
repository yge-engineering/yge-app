...
interface BidResult { id: string; jobId: string; bidOpenedAt: string; outcome: string; bidders?: Bidder[] }
interface Job { id: string; projectName: string }

export function WithMultipleBiddersTable() {
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="border-t border-gray-100">
              ...
              <td className="px-3 py-2 text-right font-mono">{(r.bidders ?? []).length}</td>
            </tr>
          ))}
        </tbody>
      </table>
