...
interface Customer { id: string; paymentTerms?: string | null }

export function ByPaymentTermsTable() {
  ...
  for (const c of customers) {
    const k = (c.paymentTerms ?? '').trim().toUpperCase() || '(unspecified)';
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map(([terms, count]) => (
            <tr key={terms} className="border-t border-gray-100">
              <td className="px-3 py-2 font-mono font-semibold">{terms}</td>
