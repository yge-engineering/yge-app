...
export function MissingBillingAddressTable() {
  ...
  const rows = customers.filter((c) => !(c.billingAddressLine ?? '').trim());

  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
        Every customer has a billing address. Nice.
      </p>
    );
  }
        ...
        <tbody>
          {rows.map((c) => (
            <tr key={c.id} className="border-t border-gray-100">
              ...
              <td className="px-3 py-2 text-xs text-gray-700">{c.city ?? '—'}</td>
              <td className="px-3 py-2 font-mono text-xs text-gray-700">{c.state ?? '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
