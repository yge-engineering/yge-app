    ...
    <table className="w-full text-sm">
      <thead className="border-b border-gray-300 text-[11px] uppercase tracking-wide text-gray-600">
        <tr>
          ...
          {cols.map((d) => (
            <th key={d} className="py-2 text-right font-mono normal-case">{d}</th>
          ))}
      ...
      <tbody className="divide-y divide-gray-200">
        {sList.map((s) => {
          ...
          const tds = cols.map((d) => {
            ...
            rowTotal += n;
            return (
              <td key={d} className="py-2 text-right text-xs font-mono">
          ...
          return (
            <tr key={s}>
              <td className="py-2 text-left font-medium text-gray-900">{s}</td>
              {tds}
