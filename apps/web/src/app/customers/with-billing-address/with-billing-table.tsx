...
export function WithBillingAddressTable() {
  ...
  const rows = customers.filter((c) => (c.billingAddressLine ?? '').trim().length > 0);

  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
        No customers with a billing address set.
      </p>
    );
  }
        ...
        <tbody>
          {rows.map((c) => (
            <tr key={c.id} className="border-t border-gray-100">
              ...
              <td className="px-3 py-2 text-xs text-gray-700">{c.billingAddressLine}</td>
              <td className="px-3 py-2 text-xs text-gray-700">{c.city ?? ''} {c.state ?? ''}</td>
            </tr>
          ))}
        </tbody>
      </table>
