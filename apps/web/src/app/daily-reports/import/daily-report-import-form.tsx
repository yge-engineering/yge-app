...
interface Summary {
  total: number; reports: number; created: number; updated: number; skipped: number;
  errors: Array<{ row: number; reason: string }>; dryRun: boolean;
}

export function DailyReportImportForm() {
  ...
  async function submit(dryRun: boolean) {
    ...
    try {
      ...
      fd.append('file', file);
      const url = `${apiBaseUrl()}/api/imported-daily-reports/import/csv${dryRun ? '?dryRun=1' : ''}`;
  ...
  return (
    <div>
      <div className="mb-4 rounded-md border border-gray-200 bg-blue-50 p-3 text-xs text-blue-900">
        <a href={`${apiBaseUrl()}/api/imported-daily-reports/export.csv`} download className="text-yge-blue-700 hover:underline">
          Download current daily report lines as a starter CSV →
        </a>
      </div>
      ...
      {result && (
        <div className="mt-3 rounded-md border border-gray-200 bg-gray-50 p-3 text-xs">
          ...
          <div>{result.total} lines · {result.reports} reports · {result.created} created · {result.updated} updated · {result.skipped} skipped</div>
