...
const LOST_STATUSES = new Set(['LOST', 'NO_AWARD']);

function yearKey(j: Job): string {
  const d = j.bidOpenedAt ?? j.startDate;
  if (!d) return 'unknown';
  const dt = new Date(d);
...
export function PrintLostByYearPanel() {
  ...
  for (const j of jobs) {
    if (!j.status || !LOST_STATUSES.has(j.status.toUpperCase())) continue;
    const k = yearKey(j);
