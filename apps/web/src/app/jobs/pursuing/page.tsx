...
export default async function PursuingJobsPage() {
  requirePermission('jobs:viewAll');
  const all = await fetchJobs();
  const rows = all.filter((j) => j.status === 'PURSUING');
  rows.sort((a, b) => (b.updatedAt ?? '').localeCompare(a.updatedAt ?? ''));

  return (
    <AppShell>
      <main className="mx-auto max-w-5xl">
        <PageHeader title="Pursuing jobs" subtitle={`${rows.length} job${rows.length === 1 ? '' : 's'} in PURSUING status.`} />
        {rows.length === 0 ? (
