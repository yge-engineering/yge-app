...
export function StatusStatsPanel() {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
      <Stat label="Top status" value={top ? top[0] : '—'} sub={top ? `${top[1]} jobs` : 'no statuses yet'} />
      <Stat label="Unique statuses" value={String(counts.size)} sub={`across ${jobs.length - missing} jobs`} />
      <Stat label="Missing status" value={String(missing)} sub={`of ${jobs.length} jobs`} tone={missing > 0 ? 'warn' : 'good'} />
    </div>
  );
}
