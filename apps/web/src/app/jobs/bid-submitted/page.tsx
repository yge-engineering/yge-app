...
export default async function BidSubmittedJobsPage() {
  requirePermission('jobs:viewAll');
  const all = await fetchJobs();
  const rows = all.filter((j) => j.status === 'BID_SUBMITTED');
  rows.sort((a, b) => (b.updatedAt ?? '').localeCompare(a.updatedAt ?? ''));

  return (
    <AppShell>
      <main className="mx-auto max-w-5xl">
        <PageHeader title="Bids submitted" subtitle={`${rows.length} job${rows.length === 1 ? '' : 's'} with a bid currently submitted (decision pending).`} />
        {rows.length === 0 ? (
          <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
            No bids currently pending decision.
          </p>
        ) : (
