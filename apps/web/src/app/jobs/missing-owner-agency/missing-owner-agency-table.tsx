...
export function MissingOwnerAgencyTable() {
  ...
  const rows = jobs.filter((j) => !(j.ownerAgency ?? '').trim());

  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
        Every job has an owner agency. Nice.
      </p>
    );
  }
        ...
        <tbody>
          {rows.map((j) => (
            <tr key={j.id} className="border-t border-gray-100">
              ...
              <td className="px-3 py-2 text-xs text-gray-700">{j.location ?? '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
