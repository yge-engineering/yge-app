...
export default async function JobsThisYearPage() {
  requirePermission('jobs:viewAll');
  const all = await fetchJobs();
  const currentYear = new Date().getFullYear();
  const rows = all
    .filter((j) => (j.createdAt ?? '').slice(0, 4) === String(currentYear))
    .sort((a, b) => (b.createdAt ?? '').localeCompare(a.createdAt ?? ''));

  return (
    <AppShell>
      <main className="mx-auto max-w-5xl">
        <PageHeader title={`Jobs created in ${currentYear}`} subtitle={`${rows.length} job${rows.length === 1 ? '' : 's'} created so far this calendar year.`} />
        {rows.length === 0 ? (
