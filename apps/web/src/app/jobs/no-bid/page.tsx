...
export default async function NoBidJobsPage() {
  requirePermission('jobs:viewAll');
  const all = await fetchJobs();
  const rows = all.filter((j) => j.status === 'NO_BID');
  rows.sort((a, b) => (b.updatedAt ?? '').localeCompare(a.updatedAt ?? ''));

  return (
    <AppShell>
      <main className="mx-auto max-w-5xl">
        <PageHeader title="Jobs we passed on" subtitle={`${rows.length} job${rows.length === 1 ? '' : 's'} we chose not to bid (NO_BID).`} />
        {rows.length === 0 ? (
