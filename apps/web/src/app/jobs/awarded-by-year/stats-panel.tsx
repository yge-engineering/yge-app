...
export function AwardedByYearTable() {
  ...
  const awarded = jobs.filter((j) => j.status === 'AWARDED' || j.status === 'ACTIVE' || j.status === 'CLOSED');
  if (awarded.length === 0) {
    return (
        ...
        <tbody>
          {rows.map(([y, count]) => (
            <tr key={y} className="border-t border-gray-100">
              ...
              <td className="px-3 py-2 text-right font-mono text-green-700">{count}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr className="border-t-2 border-gray-200 bg-gray-50 font-semibold">
            <td className="px-3 py-2">Total awarded</td>
            <td className="px-3 py-2 text-right font-mono">{awarded.length}</td>
          </tr>
        </tfoot>
      </table>
    </div>
