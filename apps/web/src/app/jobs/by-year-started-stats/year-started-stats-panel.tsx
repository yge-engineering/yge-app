...
export function YearStartedStatsPanel() {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
      <Stat label="Busiest year" value={top ? top[0] : '—'} sub={top ? `${top[1]} jobs` : 'no jobs dated yet'} />
      <Stat label="Unique years" value={String(counts.size)} sub={`across ${jobs.length - missing} jobs`} />
      <Stat label="Missing start date" value={String(missing)} sub={`of ${jobs.length} jobs`} tone={missing > 0 ? 'warn' : 'good'} />
    </div>
  );
}
