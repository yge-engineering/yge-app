...
export default async function JobsThisMonthPage() {
  requirePermission('jobs:viewAll');
  const all = await fetchJobs();
  const now = new Date();
  const yyyyMm = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
  const rows = all
    .filter((j) => (j.createdAt ?? '').startsWith(yyyyMm))
    .sort((a, b) => (b.createdAt ?? '').localeCompare(a.createdAt ?? ''));

  return (
    <AppShell>
      <main className="mx-auto max-w-5xl">
        <PageHeader title={`Jobs created in ${yyyyMm}`} subtitle={`${rows.length} job${rows.length === 1 ? '' : 's'} created so far this month.`} />
        {rows.length === 0 ? (
          <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
            No jobs created in {yyyyMm} yet.
          </p>
        ) : (
