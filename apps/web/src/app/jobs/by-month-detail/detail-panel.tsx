...
export function ByMonthDetail() {
  ...
  return (
    <div className="space-y-3">
      {sorted.map(([month, list]) => (
        <details key={month} className="rounded border border-gray-200 bg-white shadow-sm">
          ...
          <ul className="divide-y divide-gray-100 px-3 pb-2 text-sm">
            {list.map((j) => (
              <li key={j.id} className="flex items-center justify-between gap-3 py-1.5">
                ...
                <span className="font-mono text-[10px] text-gray-500">{j.jobNumber ?? ''} · {j.ownerAgency ?? ''} · {j.status ?? ''}</span>
              </li>
            ))}
          </ul>
        </details>
