...
export default async function JobsTodayPage() {
  ...
  const rows = all
    .filter((j) => (j.updatedAt ?? '').startsWith(today))
    .sort((a, b) => (b.updatedAt ?? '').localeCompare(a.updatedAt ?? ''));

  return (
    <AppShell>
      <main className="mx-auto max-w-5xl">
        <PageHeader title="Jobs touched today" subtitle={`${rows.length} job${rows.length === 1 ? '' : 's'} updated since midnight.`} />
        {rows.length === 0 ? (
          <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
            No jobs touched today (yet).
          </p>
        ) : (
