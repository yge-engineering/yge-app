...
export function WithOwnerAgencyTable() {
  ...
  const rows = jobs
    .filter((j) => (j.ownerAgency ?? '').trim().length > 0)
    .sort((a, b) => (a.ownerAgency ?? '').localeCompare(b.ownerAgency ?? ''));

  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
        No jobs with an owner agency set.
      </p>
    );
  }
