...
export function WithRateTypeTable() {
  ...
  const rows = jobs.filter((j) => (j.rateType ?? '').trim().length > 0);

  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500">
        No jobs with a rate type set.
      </p>
    );
  }
        ...
        <tbody>
          {rows.map((j) => (
            <tr key={j.id} className="border-t border-gray-100">
              ...
              <td className="px-3 py-2 font-mono text-xs text-gray-700">{j.rateType}</td>
