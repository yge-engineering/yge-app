...
export default async function LostJobsPage() {
  requirePermission('jobs:viewAll');
  const all = await fetchJobs();
  const rows = all.filter((j) => j.status === 'LOST');
  rows.sort((a, b) => (b.updatedAt ?? '').localeCompare(a.updatedAt ?? ''));

  return (
    <AppShell>
      <main className="mx-auto max-w-5xl">
        <PageHeader title="Lost jobs" subtitle={`${rows.length} job${rows.length === 1 ? '' : 's'} currently marked LOST.`} />
        {rows.length === 0 ? (
