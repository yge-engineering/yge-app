...
export default async function SubmittalsPage({
  ...
  return (
    ...
    <main className="mx-auto max-w-6xl p-8">
      ...
      <h1 className="text-3xl font-bold text-yge-blue-500">Submittals</h1>
      <p className="mt-2 text-gray-700">
        Shop drawings, product data, samples, certs sent to the engineer
        for review. Block-ordering flags surface materials that can&rsquo;t
        ship until the submittal&rsquo;s back.
      </p>
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-4">
        ...
        <Stat label="Blocks ordering" value={rollup.blocksOrderingOpen} variant={rollup.blocksOrderingOpen > 0 ? 'bad' : 'ok'} />
        <Stat
      ...
      {submittals.length === 0 ? (
        <div className="mt-6 rounded border border-gray-200 bg-gray-50 p-6 text-sm text-gray-600">
          No submittals match. Click <em>New submittal</em> to log one.
        </div>
      ) : (
                ...
                return (
                  <tr key={s.id} className={rowClass}>
                    <td className="px-4 py-3 font-mono text-sm font-bold text-gray-900">
                      {s.submittalNumber}
                      {s.revision && (
                        <span className="ml-1 text-gray-500">Rev {s.revision}</span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">
                      {s.submittedAt ?? <span className="text-gray-400">—</span>}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-700">
                    ...
                    <td className="px-4 py-3 text-right text-sm">
                      <Link href={`/submittals/${s.id}`} className="text-yge-blue-500 hover:underline">
                        Open
                      </Link>
                    </td>
                  </tr>
...
function StatusPill({ status }: { status: SubmittalStatus }) {
  const cls =
