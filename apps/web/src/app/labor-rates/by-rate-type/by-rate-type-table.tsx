...
interface Stat { rateType: string; count: number; avg: number; min: number; max: number }

function buildStats(rates: Rate[]): Stat[] {
  const map = new Map<string, { count: number; sum: number; min: number; max: number }>();
  for (const r of rates) {
    const k = (r.rateType ?? '').trim().toUpperCase() || '(unknown)';
    let s = map.get(k);
    if (!s) { s = { count: 0, sum: 0, min: Number.POSITIVE_INFINITY, max: 0 }; map.set(k, s); }
    s.count += 1;
  ...
  return [...map.entries()].map(([rateType, s]) => ({
    rateType,
    count: s.count,
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-gray-200 bg-white shadow-sm">
      <table className="w-full text-sm">
        ...
        <tbody>
          {stats.map((s) => (
            <tr key={s.rateType} className="border-t border-gray-100">
              <td className="px-3 py-2 font-semibold">{s.rateType}</td>
